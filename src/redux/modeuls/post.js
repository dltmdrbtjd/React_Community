...
const initialPost = {
    image_url: "https://m.young.hyundai.com:444/upload/CMS_NEWS_IMAGE/2020/10/22/CMS_NEWS_IMAGE_Q0t8p6urFXhUaqTGv71c.png",
    contents: "",
    like_cnt: 0,
    is_like: false,
...
const editPostFB = (post_id = null, post = {}) => {
    return function (dispatch, getState, {history}){
        ...
        const postDB = firestore.collection("coffee");
...
const addPostFB = (contents="") => {
    return function (dispatch, getState, {history}) {
        const coffeeDB = firestore.collection("coffee");
        ...
        _upload.then((snapshot) => {
            ...
            .then((url) => {
                ...
                coffeeDB.add({...user_info, ..._post, image_url: url})
                .then((doc) => {
                    ...
                    window.alert("포스트 작성에 실패했습니다.",err);
                });
            }).catch(err => {
                window.alert("이미지 업로드에 실패했습니다.",err);
            });
        });
    };
};
...
const deletePostFB = (id) => {
    return function(dispatch, getState, {history}){
        ...
        coffeeDB.doc(id).delete().then(res => {
            dispatch(deletePost(id));
            history.replace('/');
            window.alert("삭제 완료!")
        }).catch(err => {
            window.alert("삭제할 수 없습니다!",err)
        })
    }
}
